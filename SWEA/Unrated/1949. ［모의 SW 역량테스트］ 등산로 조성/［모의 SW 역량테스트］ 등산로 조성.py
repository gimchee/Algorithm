...
# 실행
1. 최대값을 찾아야 함 (봉우리 최대값)
2. 시작점 찾아야 함 (최대값 여러개 일수도 있으니, 스타팅 포인트 리스트로 만듬)
3. 스타팅 포인트를 기준으로 dfs 돌리기

# dfs 함수 만들기
1. 종료조건 먼저 -> 최대길이 구하는거니까
	 최대길이 업데이트
2. 방문행동
		델타이동으로 상화좌우 이동한다. nr,nc 업데이트
3. 벽체크 + visited 배열상에 방문 안한곳

4. 주변값이 더 낮은경우
5. 방문표시한다 + dfs 재귀호출
6. 밑에 visited = False로 원상복귀한다.

4.1 elif 깍는것을 아직 안 쓴 경우
4.2 깍기는 1 ~ K 까지 가능하므로, 반복문을 돌린다.
4.3 깍기전 위치를 저장 해 놓는다 (원상복구용)
4.4 주변값을 깍는다 (1~K 까지)
4.5 깍은 값이 현재위치보다 작은경우 -> 방문체크 + dfs(재귀)
4.6 마찬가지로 visited 원상복구 + 현재위치 arr[nr][nc] = original 원상복구(백트래킹)
